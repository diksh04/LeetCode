...
class Solution {
public:
    int helper(vector<vector<int>> &vc , int ci , int cj ,  int cuts , vector<vector<vector<int>>> &dp){
        if(cuts==0)
        {
            if(vc[ci][cj]>0)
            {
        ...
        for(int i = ci+1 ; i<vc.size()-1;i++){
            if(vc[i][cj]>=cuts && vc[ci][cj] - vc[i][cj]>0)
                rows = (rows + helper(vc,i,cj,cuts-1,dp))%M;
        }
        for(int j = cj+1 ; j<vc[0].size()-1;j++){
            if(vc[ci][j]>=cuts && vc[ci][cj] - vc[ci][j]>0)
                cols = (cols + helper(vc,ci,j,cuts-1,dp))%M;
        }
    ...
    int ways(vector<string>& pizza, int k) {
        ...
        ll ans = helper(count_apple,0,0,k-1,dp);
        return ans;
    }
};