...
public:
    TreeNode* helper(vector<int>& preorder,int preStart,int preEnd,vector<int>& postorder,int postStart,int postEnd,map<int,int>& mp)
    {
        ...
        root->left = helper(preorder,preStart+1,preStart+numsLeft+1,postorder,postStart,postRoot,mp);
        root->right = helper(preorder,preStart+numsLeft+2,preEnd,postorder,postRoot+1,
                            postEnd-1,mp);
        return root;
    }
